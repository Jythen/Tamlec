...
def load_data(cfg):
    ...
    Args:
        cfg (dict): Configuration dictionary, as given in the config file.

    Returns:
        dict: A dictionary containing dataloaders and embeddings. Depending on the method/algorithm, the keys in the 
    ...
    if not ( condition2 and condition3):
        ...
        documents = []
        labels = []
        # Can add file names that contains the data
            ...
            if file_path.exists():
                with open(file_path) as f:
                    ...
                    for idx, line in enumerate(f):
                        ...
                        # Add the labels, the label field depends on the dataset
                        
                        try:
                            ids_labels = list(data['label'])
                        except KeyError:
                            raise NotImplementedError("Check the field of the dataset where the labels are")
                        # For each label, add all its ancestors to the label set
                        ids_labels = complete_labels(ids_labels, cfg['taxonomy'].label_to_parents)
                        # If no labels, we skip the document
                        if ids_labels:
        ...
        documents_tokenized, vocabulary = tokenization(documents, cfg)
        # Split the data
        global_indices, global_relevant_labels, tasks_indices, tasks_relevant_labels = data_split(documents_tokenized, labels, cfg)
        # Get the embeddings
