...
# Compute some stats on a dataset
# Labels is a list of list containing the labels, 1 list per document
def compute_dataset_stats(output_path, labels, taxonomy):
    stats_df = pd.DataFrame(columns=['docs', 'labels', 'avg_lab/doc', 'med_lab/doc', 'avg_doc/lab', 'med_doc/lab'])
    all_labels = list(itertools.chain.from_iterable(labels))
    counts = collections.Counter(all_labels)
    # Can be that some labels in the taxonomy are not assigned -> add them to count
    all_counts = {lab: counts.get(lab, 0) for lab in taxonomy.label_to_title.keys() if lab != 'root'}
    labs_per_doc = [len(labs) for labs in labels]
    docs_per_lab = list(all_counts.values())
    dict_stats = pd.DataFrame({
        # There is one list of labels per document
        'docs': len(labels),
        # counts already get unique elements
        'labels': len(all_counts),
        'avg_lab/doc': np.mean(labs_per_doc),
        'med_lab/doc': np.median(labs_per_doc),
        'avg_doc/lab': np.mean(docs_per_lab),
        'med_doc/lab': np.median(docs_per_lab),
    }, index=[0])
    stats_df = pd.concat([stats_df, dict_stats], ignore_index=True)
    print("> Plain dataset statistics:")
    print(stats_df)
    stats_df.to_markdown(output_path / f"plain_dataset_stats.md", index=False)
...
class EarlyStopping:
    ...
    def __init__(self, max_patience, cfg, finetuning=False):
        self.cfg = cfg
    ...
    def _save_model(self, new_model):
        ...
        if self.cfg['method'] in ['hector', 'tamlec', 'attentionxml']:
            if self.finetuning:
                torch.save(new_model, self.cfg['paths']['model_finetuned'])
            else:
                torch.save(new_model, self.cfg['paths']['model'])
        # Here save the state dict of a Pytorch model than can be loaded afterwards
        else:
            if self.finetuning:
                torch.save(copy.deepcopy(new_model.state_dict()), self.cfg['paths']['state_dict_finetuned'])
            else:
